...
as
begin
    declare @dauso char(3)
	set @dauso=case abs(checksum(newid())) % 9	when 0 then '032'
												when 1 then '033'
												when 2 then '034'
												when 3 then '096'
												when 4 then '082'
...
begin
    ...
    while @dem1 <= 100
    begin
		declare @MaNCC varchar(20), @TenNCC varchar(50), @DiaChi varchar(10), @MaNV varchar(30), @TenNV varchar(50), @ktra int=0
		set @MaNCC=right ('00000'+cast(@dem1 as varchar),5)
		set @TenNCC='NCC'+cast(@dem1 as varchar)
		set @DiaChi='DC'+cast(@dem1 as varchar)
		set @MaNV='7'+right('000'+cast(@dem1 as varchar),4)
		set @TenNV='NV'+cast(@dem1 as varchar)
        while @ktra=0
        begin
        ...
        begin
            ...
            values (@maNCC, @tenNCC, @DiaChi, @so, @maNV, @tenNV)
            set @dem1 = @dem1 + 1
        end
    end
end
...
begin
	...
	begin
		...
		set @MaNoNCC='3'+right('00000000'+cast(@z as varchar),9)
